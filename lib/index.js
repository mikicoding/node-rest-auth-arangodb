...
apiRoutes.post('/register', (req, res)=> {
  ...
  ;(async ()=> {
    ...
    try {
      const { username } = doc
      const cursor = await db.query(aql`FOR x IN users FILTER x.username==${String(username)} RETURN "true"`)
  ...
  if (req.body.username && req.body.password) {
    ...
    ;(async ()=> {
      ...
      try {
        ...
        const cursor = await db.query(aql`FOR x IN users FILTER x.username==${String(username)} RETURN { username: x.username, password:x.password }`)
        const result = await cursor.next()
...
apiRoutes.get('/memberinfo', auth.authenticate(), (req, res)=> {
  ...
  if (decoded) {
    ...
    db.query(aql`FOR x IN users FILTER x.username==${String(decoded.username)} RETURN x`, (err, user, fields)=> {
